...
function setup(){
  ...
  largest = 0;
  for(let i = 0; i < scores.length; i++){
    if (scores[i] > largest){ largest = scores[i]}; //ll == i;}
  ...
  let s = smallest;// BLANK[3]　ヒント：最小値とだいたい同じ
  console.log(s);
  ...
  for(let i = 0; i < scores.length; i++){
    ...
    const h = height * scores[i] / 100;
     ...
     //rect(l* dx +2, height - (height*l/100), dx-4,(height*l/100));
     //if (){}

     //rect(s* dx +2, height - (height*s/100), dx-4,(height*s/100));
